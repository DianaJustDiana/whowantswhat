...
def index(request):
    ...
    # .values('description', 'photo', 'dib__owner__username')
...
def new_offer(request):
    ...
    if request.method != 'POST':
        ...
        form = OfferForm(current_user)
    else:
        # POST data submitted; process data.
        # Three bits here -- request.user for ownership, request.POST for text, request.FILES for images.
        # The order here is IMPORTANT. The user part has to come first.
        form = OfferForm(request.user, request.POST, request.FILES)
...
def available_to_me(request):
    ...
    # .values('dib__owner__username', 'description', 'photo', 'owner__username', 'owner__family__family_name', 'id')
    offers = Offer.objects.filter(family__member__name=current_user)
    print("offers:")
    print(offers)
...
def add_dib(request):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            current_user_called_these_dibs = Offer.objects.filter(
                dib__owner=current_user)
            print("Test me")
...
def all_my_dibs(request):
    ...
    my_dibs = Offer.objects.filter(dib__owner=current_user)
...
def dibs_on_my_stuff(request):
    ...
    offers = Offer.objects.filter(
        dib__owner__member__family__parent=current_user).distinct()
